...
Pool.prototype._reset = function () {
    ...
    this.maxS = this.peers.map( peer => peer.weight ).reduce( ( a, b ) => {
        return a > b ? a : b;
    });

    this.gcdS = this.peers.map( peer => peer.weight ).reduce( ( prev, cur ) => {
        return gcd( prev, cur );
    });
...
Pool.prototype.get = function (predicate) {
    var i = this.peers.map( peer => peer.value ).findIndex( predicate );
